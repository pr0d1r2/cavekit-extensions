...
| T8 | x | **`ck:spec` audit-mode skill** — default-on, skill-judged proportional ceremony: full decision-unit review/commits, light one-WHY commit, or none for meaning-preserving edits; no user tier flag; dirty-spec + hook-failure safety. + bats. | V8,C8,I.ck-spec-audit |
...
| B7 | 2026-07-14 | CI: `guardrails.yml@main` runs `nix run .#confirm`, but the vendored→referenced migration left the flake hand-rolling lefthook wrappers and never exposed `apps.confirm` nor materialized `lefthook.yml` → `flake does not provide attribute 'apps.x86_64-linux.confirm'` | Rewrite flake to the referenced leaf standard: derive `lefthook.yml` + wrapper set from `set-and-setting.lib.materializationFor`, add the `apps.confirm` acceptance gate, and have the devShell copy `lefthook.yml`; prune the now-unused vendored wrapper inputs from `flake.lock` |
| B8 | 2026-07-14 | CI: `nix flake check` `shfmt-check` fails — `ck-*.sh` use 2-space indent but the pinned `lefthook-shfmt` wrapper enforces `-i 4 -ci` (4-space, indent case bodies) | Reformat all `ck-*.sh` with `shfmt -i 4 -ci -w` (whitespace-only, semantics-preserving; 157 bats pass); 4-space still satisfies `.editorconfig` `indent_size=2` (multiple of 2) |
| B9 | 2026-08-04 | CI: strict flake-manifest guard rejects inline `outputs` construction; the shared `set-and-setting` input also creates a duplicate `nixpkgs` lock node | Delegate outputs to `set-and-setting.lib.mkConsumerFlake` and make its `nixpkgs-lock` input follow the root lock |
| B10 | 2026-08-17 | CI: `nix flake check` actionlint evaluation fails because the shared helper passes a string to Nixpkgs `sourceByRegex`, which now requires a list | Keep the actionlint check but provide the workflow source filter as a list and register it through `extraChecks` |
| B11 | 2026-08-17 | CI: `nix flake check` `nixfmt-check` fails because the actionlint source filter in `flake.nix` is not formatted in the pinned Nix style | Format the source filter as a multiline list |
